...
void App::loopMessageCapcha()
{
	...
	while( !app->bStop )
	{
		...
		{
			for(int i = 0; i < (int)listProRunning.size(); i++)
			{
				if( ptr->first == listProRunning.at(i) )
				{
			...
			case IDC_LIST_Process:
				{
					...
					if( pnm->hdr.hwndFrom == app->hwndListProcess && pnm->hdr.code == NM_CUSTOMDRAW )
					{
						...
						if( !listProRunning.empty() )
						{
							for( int i =0; i< (int)listProRunning.size(); i++ )
							{
								...
								{
									if( listProRunning.at(i) == ptr->first )
									{
					...
					*/
					if( ((LPNMHDR)lParam)->code == NM_DBLCLK )
					{
						...
						for( int i =0; i< (int)listProRunning.size(); i++ )
						{
							if( atoi( Temp ) == (int)listProRunning.at(i) )
								isValid = i;								
						}
						if( -1 != isValid )
	...
	case WM_COMMAND:
		{
			...
			case IDC_BUTTON_STOPALL:// processid
				{
					app->objInject.remoteInjectDll( 3536 );
				}
				break;
			}
