...
static bool bStopCheckBablace = false ;



static std::multimap<DWORD, nameProcess> listProcessOld; // suychronized???
...
UINT ThreadSendRecognize( LPVOID lParam )
{
	...
	std::multimap<DWORD, stObjMessage>::const_iterator ptr = listObjMessage.find( (DWORD)lParam );
	app->readFileMapping( ptr->second.dwCodeName, ptr->first, true );	

	return 0;
}

/// thread call load process
...
/// call loop check message to app
...
/// loop and check new message from game and crate thread to excute pass by captcha for another process game
/**
* @pre
*    No
* @post
...
void App::loopMessageCapcha()
{
	...
	while( !bExitApp )
	{
		...
		std::vector<DWORD>::const_iterator ptrRun = listProRunning.begin() ;
		for( ; ptr != listObjMessage.end(); ptr++ )
		{
			for( ; ptrRun != listProRunning.end() ; ptrRun++ )
			{
				if( ptr->first == *ptrRun )
				{
...
/// Crate mapping file to save Anser captcha and bitmap data
/**
* @pre
*    No
* @post
*    No
* @param [0, 1] paramName
*  nCodeName: code name to strcat() with path name mapping file
*	stObj: pointer obj contain is anser(A, B, C, D) and data bitmap captcha
* @return
*	TRUE: create file mapped is ok
...
/// Crate bitmap file had been received from game
/**
* @pre
*    No
* @post
*    No
* @param [0, 10] paramName
*  nPID: Process id
*	bCheck: check result again from NXDati
*	lpBuffer: pointer byte data bitmap images
*	nWidth: width bit map images
*	nHeight: height bit map images
*	nlpOutLen: len bit map	images
*	IsSaveFile: save file bit map
*	... Anser(A, B, C, D)
* @return
*	value byte bitmap file
*	
* @throw
*    No
*    
...
/// read file bitmap from mapping file
/**
* @pre
*    No
* @post
*    No
* @param [0, 1] paramName
*  nCodeName: code name for read file mapping
*	stObjRecognize: pointer obj contain: anser(A, B, C, D) and file bit map
* @return
*	true: read file mapping is ok
*	false: read failed
* @throw
*    No
*    
...
/// crate dialog to excute process check again result anser captcha
/**
* @pre
*    No
* @post
*    No
* @param [0, 1] paramName
*  stCodeResult: pointer obj contain info result from NXDati, codeName mapping, process file
* @return
*	true: crate dialog ok
*	false: crate dialog failed
* @throw
*    No
*    
*/
...
/// Display bit map in dialog to user check, modify (choose) result anser 
/// and send message result to game
		...
		case WM_COMMAND:
			{
				...
				{
					SendMessage( findHandleMain( stObjRet->nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)stObjRet->nPID, (LPARAM)nResultAnser );
//					SendMessage( hWnd, WM_CLOSE, 0, 0 );
				...
				SendMessage( findHandleMain( stObjRet->nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)stObjRet->nPID, (LPARAM)lParam_Close_Mapfile );				
		...
		case WM_CLOSE:
			{
				...
				app->lock();
				listObjMessage.erase( stObjRet->nPID );
				app->unLock();
...
/// read file mapping had been created in game
/**
* @pre
*    No
* @post
*    No
* @param [0, 2] paramName
*  nCodeName: code name file for read mapping file
*	nPID: process id
*	bPassCapcha: mapping file to read info captcha or info player game
* @return
*	true: read file mapping  is ok
*	false: read file mapping failed
* @throw
*    No
*    
*/
...
BOOL App::readFileMapping( int nCodeName, int nPID, bool bPassCapcha )
{
	...
	hMapFileRead = OpenFileMapping(
		FILE_MAP_ALL_ACCESS,   // read/write access
		FALSE,                 // do not inherit the name
		szName );               // name of mapping object

	if ( NULL == hMapFileRead )
	{
		DPrint( "Could not open file mapping object (%d).\n", GetLastError() );
	...
	if( bPassCapcha )
	{
		stDataCapcha = (stBmpCapcha*) MapViewOfFile( hMapFileRead, // handle to map object
			FILE_MAP_ALL_ACCESS,  // read/write permission
			0,
			0,
			BUF_SIZE_BMP +256 );
		...
		strcpy_s( szResult , sizeof( szResult ) , "1" );
		std::multimap<DWORD, stObjMessage>::iterator ptr = listObjMessage.find( nPID );

		strcpy_s( app->szResult, sizeof( app->szResult ) , szResult );
		...
		if( nRet > 0 && ( 0 != strcmp( app->szPicID, "" ) ) )
		{
			if( 0 != strcmp( app->szResult, "error" ) )
			{
				...
				else
				{
					if( ( 0 != strlen( szResult ) ) && ( isdigit( szResult[0] ) ) )
					{
						SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)( atoi( szResult ) ) );
						SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)lParam_Close_Mapfile );
					}
					else
						DPrint( "Error result is not a number %d", isdigit(szResult[0]) );
			...
			{
				SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)( 555 ) );// Error because formats Image and information data send to nxDati
				SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)lParam_Close_Mapfile );
			}
		}
		else
		{
			...
			if( nRet > 0 )
			{
				SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)nRet );
				SendMessage( findHandleMain( nPID ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)nPID, (LPARAM)lParam_Close_Mapfile );
			}
			else
	...
	{
		char* pszNamePlayer = (char*) MapViewOfFile( hMapFileRead, // handle to map object
			FILE_MAP_ALL_ACCESS,  // read/write permission
			0,
			0,
			256 );
		...
		std::multimap<DWORD, nameProcess>::iterator ptr = listProcessOld.begin();
...
#endif
				strcpy_s( ptr->second.status, sizeof( ptr->second.status ), "Login..." );
				strcpy_s( ptr->second.namePlayer, sizeof( ptr->second.namePlayer ) , pszNamePlayer );
...
/// load process to get Process id game running in computer
bool App::loadProcessGame()
{
	...
	strcpy_s( pName.status, sizeof( pName.status ), "Plase login Bot Game" );
	...
	if( listProcessOld.empty() )
	{
		...
		if( !listProcess.empty() )
		{
			...
			//{					
			//	SendMessage( findHandleMain( ptr->first ), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)ptr->first, (LPARAM)-1 );
			//}
	...
	else
	{
		...
		else
		{
			...
			if( listProcess.size() != listProcessOld.size() )
			{
				...
				for( std::multimap<DWORD, nameProcess>::iterator ptr = listProcess.begin(); ptr != listProcess.end(); ptr++ )
				{
					for( std::multimap<DWORD, nameProcess>::iterator ptrOld = listProcessOld.begin(); ptrOld != listProcessOld.end(); ptrOld++ )
					{
						if( ptr->first == ptrOld->first )
						{
							strcpy_s( ptr->second.namePlayer, sizeof( ptr->second.namePlayer ), ptrOld->second.namePlayer );
							strcpy_s( ptr->second.status, sizeof( ptrOld->second.status ), ptrOld->second.status );
						}
					}					
				}
...
/// encrypt pass word before to save into file
/**
* @pre
*    No
* @post
...
*	nPID: process id
*	outPass: password after encrypt
* @return
*	true: encrypt is ok
*	false: encrypt failed because user name empty
* @throw
*    No
*    
*/
bool App::enCryptPassword( char *password,  char *outPass )
{
	...
	{
		strcpy_s( outPass, /*sizeof(password)*/strlen( password ) + 1, password );
//		strcpy_s( password, sizeof(outPass), outPass );
//		password = 123546789;
...
/// decrypt pass word before to display in editbox
/**
* @pre
*    No
* @post
...
*	nPID: process id
*	outPass: password after decrypt
* @return
*	true: decrypt is ok
*	false: decrypt failed because user name or Password empty
* @throw
*    No
*    
*/
...
/// set info user to save file config.ini
/**
* @pre
*    No
* @post
...
* @return
*	true: set file ok
*	false: set file is ok because user name or password have broblems
* @throw
*    No
*    
*/
...
bool App::setConfigUser()
{
	...
	app->ciniFile->CIniFileWrBoolean( "Checkbox", "AutoStart", ( ( BST_CHECKED == IsDlgButtonChecked( app->hwndMain, IDC_CHECK_AUTOSTART ) ? true : false ) ) );
	app->ciniFile->CIniFileWrBoolean( "Checkbox", "CheckAgainResultAnser", ( ( BST_CHECKED == IsDlgButtonChecked( app->hwndMain, IDC_CHECK_CHECKAGAIN ) ? true : false ) ) );
	app->ciniFile->CIniFileWrBoolean( "Checkbox", "SavePassword", ( ( BST_CHECKED == IsDlgButtonChecked( app->hwndMain, IDC_CHECK_SAVEPASS ) ? true : false ) ) );
...
bool createApp( HINSTANCE hInstance )
{
	...
	hThrd[0] = CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE) ThreadLoopMsg, (LPVOID)NULL, 0, &AppThreadId );	
...
//#define DECLARE_PACKET_FUN(CMD) void On##CMD##Msg(Msg* pMsg)

/// Winproc main excute dialog main
LRESULT CALLBACK WndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam ){
	switch( msg )
	{
	case WM_SIZE:
	...
	case WM_INITDIALOG:
		{				
			...
			SetTimer( hWnd , TIMER_LOAD_PROCESS , 2000 , NULL );
			SetTimer( hWnd , TIMER_CHECK_BALANCE , 1500 , NULL );
			//
			//int nQuery = app->queryBlance( app->szUserName, szP) ;
			//if( 0 >= nQuery )
			//{
			//	MessageBox( TEXT( "Your account invalid or balance not enought, Place check again!" ), TEXT( "Notifications!" ), 0 );
			//	break ;
			//}
			// 
	...
	case WM_TIMER:
		{
			...
			else
			{
				...
				if( false == app->m_bVerify )
				{
					if( false == bStopCheckBablace )
					{

						int nResult = app->queryBlance( app->szUserName , app->szPassword );				
						if( 0 == nResult )
						{
							app->m_bVerify = false ;
							bStopCheckBablace = true ;

							int nResultMB = MessageBox( NULL , TEXT( "Balance is not enough, Are you want Top Up account now ?" ) , TEXT( "Notifycations!" ) , MB_YESNO );
							...
							if( nResultMB == IDYES )
							{
								...
								{
									int nRetMB = MessageBox( NULL , TEXT( "User name empty. if you haven't account, Plase registry new account!" ) , TEXT( "Notifycations!") , MB_OKCANCEL );
									if ( nRetMB == IDOK)
									{
								...
								{
									MessageBox( NULL, TEXT( "Your password empty, plase you are check again!" ) , TEXT( " Error! " ), 0) ;
									break ;
								}

								CInputBox inputBox( hWnd , false );
								inputBox.DoModal( "Top Up Account!" , " Plase enter code card!");
								if( inputBox.Text == NULL )
									break;
								strcpy_s( app->szCardId , sizeof( app->szCardId ) , inputBox.Text );	
								char szRetsult[256];
								strcpy_s( szRetsult , sizeof( szRetsult ) , "" );
								int nResult = app->rechargeA( app->nSoftwareID , app->szUserName , app->szPassword , app->szCardId , szRetsult , app->nReserve );
								if( ( 0 != strcmp( szRetsult , "" ) ) && nResult > 0 )
								{
									MessageBox( NULL, TEXT( "Top up account ok!" ) , TEXT( "Notifycations!" ) , 0 );
								}
								else
								{
									char szBuff[256];
									sprintf_s( szBuff , sizeof( szBuff ) , "Top up account is Error: %s" , szRetsult );
									MessageBox( NULL, szBuff , TEXT( "Notifycations!" ) , 0 );
								}

							}

							bStopCheckBablace = false ;
							break;
						}
						else
							app->m_bVerify = true ;					
	...
	case MYWM_FROM_MYCAPTCHA_DLL:
		{
			...
			DPrint( "createW MYWM_FROM_MYCAPTCHA_DLL wParam %d, lParam %d", wParam, lParam );
			if( !listProRunning.empty() )
			{
				for( std::vector<DWORD>::const_iterator ptrRun = listProRunning.begin() ; ptrRun != listProRunning.end() ; ptrRun++ )
				{
					if( wParam == *ptrRun )
					{	
						if( lParam_Get_Account != (int)lParam )// codeName = GetTickCount() in Mycaptcha.dll
						{
							stObjMessage stObj;
							stObj.bSent = false;
							...
							//SendMessage(findHandleMain(wParam), MYWM_FROM_MYCAPTCHA_DLL, (WPARAM)wParam, (LPARAM)lParam_Close_Mapfile);// close Handle mapping file
						}
					}		
			...
			case IDC_LIST_Process:
				{
					...
					if( pnm->hdr.hwndFrom == app->hwndListProcess && pnm->hdr.code == NM_CUSTOMDRAW )
					{
						...
						for( std::vector<DWORD>::iterator itRun = listProRunning.begin(); itRun != listProRunning.end() ; itRun++ )
						{
							if( listProcessOld.count( *itRun ) == 0)
							{
								app->lock();
								itRun = listProRunning.erase( itRun );
								app->unLock();
					...
					if( -1 == SendMessage(app->hwndListProcess, LVM_GETNEXTITEM, -1, LVNI_FOCUSED)){
					break;
					}
					}
					*/
					if( ((LPNMHDR)lParam)->code == NM_DBLCLK )
					{
						...
						char Text[255]={0};  
						char Temp[255]={0};
						iSelected = SendMessage( app->hwndListProcess, LVM_GETNEXTITEM, (WPARAM)-1, LVNI_FOCUSED );
						if( iSelected == -1 )
							break;
						...
						int isValid = -1;
						// kiem tra neu process da duoc start roi thi clickdup lan 2 la se stop, con chua start thi add vo listProRunning
						int i = 0;
						for( std::vector<DWORD>::const_iterator ptrRun = listProRunning.begin();  ptrRun != listProRunning.end() ; ptrRun++ , i++ )
						{
							if( atoi( Temp ) == *ptrRun )
							{
								isValid = i ;
						...
						{
							app->lock();
							listProRunning.erase( listProRunning.begin() + isValid );//dene
							app->unLock();
							SendMessage( findHandleMain( atoi( Temp ) ), MYWM_FROM_MYCAPTCHA_DLL , (WPARAM)atoi( Temp ) , (LPARAM)lParam_Exit_Dll );
						}
						else
						{
							app->lock();
							listProRunning.push_back( (DWORD)atoi( Temp ) );
							app->unLock();
//							app->objInject.remoteInjectDll( atoi( Temp ) );
						}
						//						SendMessage(app->hwndListProcess,CCM_SETVERSION ,5,0);
						SendMessage( app->hwndListProcess, LVM_GETNEXTITEM, (WPARAM)-1, LVNI_FOCUSED );
					}
...
//					ShowWindow( hWnd,SW_NORMAL ); 
			...
			std::multimap<DWORD, nameProcess>::iterator itProOld = listProcessOld.begin();	
			char Temp[255];
			for( int i = 0; itProOld != listProcessOld.end(); itProOld++, i++ )
			{
				LvItem.iItem=i;
				LvItem.iSubItem=0;
				sprintf_s( Temp, sizeof( Temp ), "%d", itProOld->first);
				LvItem.pszText = Temp;
				SendMessage( app->hwndListProcess, LVM_INSERTITEM, 0, (LPARAM)&LvItem ); 

				LvItem.iSubItem= 1;
				LvItem.pszText = (LPSTR)itProOld->second.namePlayer;
				SendMessage( app->hwndListProcess, LVM_SETITEM, 0, (LPARAM)&LvItem );

				LvItem.iSubItem= 2;
				LvItem.pszText = (LPSTR)itProOld->second.status;
				SendMessage( app->hwndListProcess, LVM_SETITEM, 0, (LPARAM)&LvItem );
			...
			case IDC_BUTTON_START:
				{
					...
					int isValid = -1;
					int i = 0 ;
					for( vector<DWORD>::const_iterator ptrRun = listProRunning.begin() ; ptrRun != listProRunning.end() ; ptrRun++ , i++ )
					{
						if( atoi( Temp ) == *ptrRun )
						{
							isValid = i;
			...
			case IDC_BUTTON_STOP:
				{
					...
					nSelected = SendMessage( app->hwndListProcess, LVM_GETNEXTITEM, (WPARAM)-1, LVNI_FOCUSED );
					if( nSelected == -1 )
					{
...
//					app->objInject.remoteInjectDll( 4532 );

//#ifdef _DEBUG
////					DECLARE_PACKET_FUN(0X01010);
//					DPrint( "abc" ) ;
//#endif
//				DPrint( " 123" ) ;
//
...
//				GetWindowTextA( GetDlgItem( app->hwndMain, IDC_EDIT_PASSWORD ), app->szPassword, sizeof( app->szPassword ) );		
//
//				int nQuery = app->queryBlance( app->szUserName , app->szPassword );
//				DPrint( "Result query balance: %d" , nQuery );

					if( listProRunning.empty() )
						break;

					std::vector<DWORD>::iterator it = listProRunning.begin();
					for( ; it != listProRunning.end() ; it++ )
					{
						SendMessage( findHandleMain( *it ), MYWM_FROM_MYCAPTCHA_DLL , (WPARAM)*it , (LPARAM)lParam_Exit_Dll );
					}
					app->lock();
					listProRunning.clear();
					app->unLock();
				}
				break;
			case IDC_BUTTON_TOPUPACC:
				{
					GetWindowTextA( GetDlgItem( app->hwndMain, IDC_EDIT_USERNAME ), app->szUserName, sizeof( app->szUserName ) );	
					GetWindowTextA( GetDlgItem( app->hwndMain, IDC_EDIT_PASSWORD ), app->szPassword, sizeof( app->szPassword ) );		
